...
def load_config(path):
    ...
    allowed = {'description', 'mode', 'word_count', 'pool', 'pool_file', 'fixed_positions',
               ...
               'index_start', 'index_count', 'include_master', 'compressed_modes'}
...
def run(args):
    ...
    if args.describe:
        ...
        return 0
    if not 0 < args.duty <= 1 or not 1 <= args.workers <= (os.cpu_count() or 1):
...
def main():
    ...
    parser.add_argument('--config', type=Path, default=ROOT / 'puzzle-search.json')
    ...
    parser.add_argument('--duty', type=float, default=0.65, help='Active fraction per worker; default .65')
    parser.add_argument('--full-cpu', action='store_true',
                        help='Use all logical CPUs (up to 61 on Windows), duty=1; overrides workers/duty')
    ...
    try:
        ...
        if args.full_cpu:
            args.workers = min(os.cpu_count() or 1, 61 if os.name == 'nt' else (os.cpu_count() or 1))
            args.duty = 1.0
        return run(args)
